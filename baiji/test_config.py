...
class TestAWS(unittest.TestCase):
    ...
    def test_credentials_set_aws_keys_and_default_region(self):
    ...
    def test_credentials_set_region_from_env(self):
        from baiji.config import credentials
        env = EnvironmentVarGuard()
        env.set('AWS_DEFAULT_REGION', 'ap-northeast-1')

        with env:
            self.assertEqual(credentials.region, 'ap-northeast-1')
